...
function createPgPool() {
  ...
  if (!url && !hasDirectConfig) {
    console.log(
      "No database configuration found - running without database features",
    );
    return null;
  }
  ...
  try {
    ...
    pool
      ...
      .catch((err) => {
        console.error(
          "Database connection failed, but continuing without database features:",
          err.message,
        );
      });
    ...
    console.error(
      "Failed to create database pool, continuing without database features:",
      error.message,
    );
    return null;
  }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!anthropic) {
        console.error(
          "❌ Anthropic SDK not available - email editing disabled",
        );
        return res
          .status(503)
          .json({ error: "Email editing service not available" });
      }
      ...
      if (testMode) {
        console.warn(
          "⚠️ WEBHOOK_TEST_MODE enabled - signature verification bypassed",
        );
        ...
        console.warn(
          "⚠️ RESEND_WEBHOOK_SECRET set but no signature header - rejecting",
        );
        console.warn(
          "   Set WEBHOOK_TEST_MODE=true to bypass signature for testing",
        );
        return res.status(401).json({ error: "Missing signature" });
      } else {
        console.warn(
          "⚠️ RESEND_WEBHOOK_SECRET not set - webhook signature verification disabled",
        );
      }
      ...
      if (body.type === "email.received") {
        ...
        if (emailId) {
          console.log(
            "📥 Fetching email content from Resend API, email_id:",
            emailId,
          );
          try {
            ...
            if (emailData.headers)
              headers = { ...headers, ...emailData.headers };
          } catch (error) {
            console.error(
              "❌ Failed to fetch email content from Resend API:",
              error.message,
            );
          ...
          console.warn(
            "⚠️ No email_id found in webhook event - cannot fetch email body",
          );
          console.log(
            "📦 Event structure:",
      ...
      if (!text && !html) {
        ...
        return res
          .status(200)
          .json({ message: "Email has no body content, skipping" });
      }
      ...
      const editEmailAddress =
        process.env.EDIT_EMAIL_ADDRESS || "edit@aimuseum.se";
      const finalMailbox =
        process.env.EDIT_FINAL_MAILBOX || "admin@aimuseum.se"; // All Hostinger aliases forward here
      ...
      if (Array.isArray(to)) {
        recipientEmail =
          typeof to[0] === "string" ? to[0] : to[0]?.email || to[0]?.address;
      ...
      const allRecipients = [
        ...
        .map((e) => {
          ...
          const emailMatch = String(e).match(
      ...
      const isEditAddress = allRecipients.some((r) =>
        r.includes(editEmailLower),
      );
      const isFinalMailbox = allRecipients.some((r) =>
        r.includes(finalMailboxLower),
      );
      ...
      const isValidRecipient =
        isEditAddress ||
        (isFinalMailbox && editEmailLower !== finalMailboxLower);
      ...
      if (!isValidRecipient) {
        ...
        return res
          .status(403)
          .json({ error: "Email must be sent to dedicated edit address" });
      }

      if (isEditAddress) {
        console.log(
          "✅ Email sent to edit address (original recipient):",
          editEmailLower,
        );
      } else if (isFinalMailbox) {
        console.log(
          "✅ Email sent to final mailbox (Hostinger alias forwarding):",
          finalMailboxLower,
        );
        console.log(
          "  Original recipients in headers:",
          allRecipients.filter((r) => r.includes(editEmailLower)),
        );
      }
      ...
      if (!allowedEmails.length) {
        console.error(
          "❌ ALLOWED_EDITOR_EMAILS not configured - rejecting all requests",
        );
      ...
      if (requiredToken) {
        const tokenInSubject = subjectLower.includes(
        ...
        console.warn(
          "⚠️ EDIT_SECRET_TOKEN not set - token verification disabled",
        );
      }
...
function extractEditRequest(emailBody) {
  ...
  if (sectionMatch) {
    ...
    const contentMatch = emailBody.match(
      /\[([^\]]+)\]\s*([\s\S]*?)\s*(?:\[\/\1\]|$)/,
    );
...
async function analyzeEditWithAI(editRequest) {
  ...
  const contentSummary = currentContent
    .slice(0, 50)
    .map((row) => `${row.key} (${row.lang}): "${row.body.substring(0, 60)}..."`)
    .join("\n");
...
app.get("/api/webhook/email/test", (req, res) => {
  ...
  res.json({
    ...
    warnings: [
      ...
      editEmailAddress.toLowerCase() === finalMailbox.toLowerCase() &&
        "ℹ️ EDIT_EMAIL_ADDRESS and EDIT_FINAL_MAILBOX are the same - no alias forwarding detected",
    ].filter(Boolean),
  });
});
...
app.get("*", (req, res) => {
  // Only handle requests that don't start with /api/
