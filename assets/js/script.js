...
document.addEventListener('DOMContentLoaded', () => {
    // Derive brand gradient colors from header logo and apply to CSS variables
    (function deriveBrandFromLogo() {
        const logo = document.querySelector('header img[src*="mai-logos/"]');
        if (!logo) return;
        ...
        function toHex(n) { const v = Math.max(0, Math.min(255, Math.round(n))); return v.toString(16).padStart(2, '0'); }
        ...
        function extractPalette(image) {
            try {
                ...
                const target = 80; // downscale width
                ...
                for (let i = 0; i < data.length; i += step) {
                    ...
                    const sum = r + g + b;
                    if (sum < 90) continue; // ignore near-black
                    // quantize to 24 levels/channel
                    const qr = Math.round(r / 11) * 11;
                    const qg = Math.round(g / 11) * 11;
                    const qb = Math.round(b / 11) * 11;
                ...
                const entries = [...map.entries()].map(([key, count]) => {
                    ...
                    return { r, g, b, count, hsl };
                });
                // prefer vivid mid/bright colors
                const filtered = entries
                    .filter(c => c.hsl.s >= 0.35 && c.hsl.l >= 0.35 && c.hsl.l <= 0.75)
                    .sort((a, b) => b.count - a.count);
                const top = filtered.slice(0, 12);
                if (top.length === 0) return null;
                // pick three with distinct hues
                const selected = [];
                const hueDistance = 30; // degrees
                for (const c of top) {
                    if (selected.every(s => Math.abs(s.hsl.h - c.hsl.h) > hueDistance)) {
                        selected.push(c);
                        if (selected.length === 3) break;
                    }
                }
                while (selected.length < 3 && filtered[selected.length]) selected.push(filtered[selected.length]);
        ...
        function applyVars([start, mid, end]) {
            const root = document.documentElement;
            if (start) root.style.setProperty('--brand-start', start);
            if (mid)   root.style.setProperty('--brand-mid', mid);
            if (end)   root.style.setProperty('--brand-end', end);
        }

        function run() {
            const colors = extractPalette(logo);
            if (colors && colors.length >= 3) applyVars(colors);
        }
        if (logo.complete && logo.naturalWidth) run(); else logo.addEventListener('load', run, { once: true });
    })();
